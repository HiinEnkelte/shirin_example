...
public class CLDA implements Serializable {
                     ...
                     Randoms r) throws IOException
	{
		...
		//System.out.println("k:"+tmpCnt+ " R:");
        	truevocConcepts[tmpCnt]=vocConcepts[tmpCnt];
        }
		...
		while ((tempString = proWordConceptReader.readLine()) != null){
			...
			if(temp.length==1)
			{
				...
				for (int i = 1; i < temp.length; i++) {
					String[] pro1=temp[i].split(" ");
					String[] pro=new String[2];
					//pro[1]=pro1[pro1.length-1];
					String h="";
					for (int k = 0; k < pro1.length-1; k++) {																													h=h+ " "+ pro1[k];
		  			//System.out.println("AMI:"+h);				
					}
			//System.out.println(h);		
					...
					System.out.println("Word: "+pro[0]+ " KL "+conceptMap.get(pro[0]));
					numProbabilityWordConcept[wordInd][i-1]=conceptMap.get(pro[0]);
					System.out.println("LL: "+conceptMap.get(pro[0]));
